    ...
    try {
      ...
      if (userIds.length > 0) {
        const { data, error: usersError } = await supabase
          .from('users')
          .select('id, full_name, voucher_code, company_id, department, position, company:companies(id, name), shift:shifts(id, name, start_time, end_time)')
          .in('id', userIds);
  ...
  const exportToPDF = () => {
    try {
      ...
      const tableData = meals.map(meal => {
        ...
        // Para refeições extras: mostrar turno para funcionários e N/A para visitantes
        // Para refeições regulares: sempre mostrar o turno do usuário
        const isVisitor = (meal as any).external_name;
        const turno = isVisitor ? 'N/A' : (meal.user?.shift?.name || 'N/A');
